...
# This version requires the user to give the api keys in the sidebar.
# However, it is not saved when the user switches between pages Each time the user has to enter the keys again. --> to be fixed
...
@st.cache_resource
def get_as_api_key():
    return {'anysc_key': None}

def intitialize(youtube_key):
    global youtube, summarizer, prefix, summary_suffix
...
def llama_get_summary(docs, name, ANYSC_API_KEY):
    summary_all = ''
    ...
    if os.path.isfile(file_name):
        ...
        for index, item in enumerate(docs):
            if index < len(item):
                response = llama_summarizer(item, ANYSC_API_KEY)
...
def main():
    ...
    if "youtube_key_user" not in st.session_state:
        st.session_state.youtube_key_user = ""
    
    if "anysc_key_user" not in st.session_state:
        st.session_state.anysc_key_user = ""   
    
    # Create a text input widget
    st.sidebar.write("Enter your APIs key below:")
    youtube_key_user = st.sidebar.text_input("Youtube key", value=st.session_state.youtube_key_user)
    anysc_key_user = st.sidebar.text_input("Anyscale key", value=st.session_state.anysc_key_user)
    
    # Store the current value of the widget in session state
    st.session_state.youtube_key_user = youtube_key_user
    st.session_state.anysc_key_user = anysc_key_user
    
    if youtube_key_user and anysc_key_user:
        ...
        get_yt_api_key()['youtube_key'] = youtube_key_user
        get_as_api_key()['anysc_key'] = anysc_key_user
              
                
        youtube_key = get_yt_api_key()['youtube_key']
        anysc_key = get_as_api_key()['anysc_key']
        intitialize(youtube_key)
            ...
            for index, item in enumerate(result['items']):
                ...
                if os.path.isfile(current_summary_file_name):
                    ...
                    if transcript is not None:
                        ...
                        summary_all = llama_get_summary(text_chunks,current_summary_file_name, anysc_key)
                        # stop the timer
                        end_time = time.time()
        ...
        st.write("Please provide your API keys for Youtube and Anyscale to use this app.")
